	...
	for entity in entities {
		...
		match entity.kind {
			MessageEntityKind::Url => {
				...
				// Telegram should never send URLs that are not parsable,
				// but there is one special case with URLs
				// that are missing a protocol (scheme).
				let Ok(mut url) = parse_with_default_scheme(utf8) else {
					log::error!(
						"Unable to parse the URL from a message: {}",
						utf8
					);
					continue;
				};
				if matches!(try_mutate_url(&mut url), Mutated) {
					...
					// Compute how much the URL length changed
...
impl Rule for UrlHostReplaceRule {
	fn apply(
		...
		for_each_url_mut(text, entities, |url: &mut Url| {
			...
			// Missing hosts indicate malformed input or
			// unexpected entity data.
			...
			// Avoid producing invalid URLs by rejecting
			// invalid replacement hosts.
